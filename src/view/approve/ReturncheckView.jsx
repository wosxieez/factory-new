...
export default props => {
    ...
    const listData = useCallback(async (conditionObj) => {
        ...
        // console.log('sql:', sql)
        let result_count = await getCountRT(sql_condition)
        setCount(result_count)
        if (hasPermission6) { appDispatch({ type: 'returncount', data: result_count }) }
        let result = await api.query(sql)
        if (result.code === 0) {
            let tempSumcount = 0;
            let tempSumprice = 0;
    ...
    const columns = [
        { title: '退料时间', dataIndex: 'date', key: 'date', width: 120, align: 'center', fixed: 'left', },
        {
            title: '单号/摘要',
            dataIndex: 'code_num',
            key: 'code_num',
