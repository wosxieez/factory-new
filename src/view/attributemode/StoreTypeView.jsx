...
const { confirm } = Modal
export default function StoreTypeView() {
    ...
    const init = useCallback(async () => {
        let res = await HttpApi.getStoreAttributeList({ table_index: 1 })
        if (res.code === 0) {
    ...
    const addHandler = useCallback(async (data) => {
        ...
        const obj = { pid, name: data.title, table_index: 1 }
    ...
    const updateHandler = useCallback(async (data) => {
        ...
        const obj = { pid: data.pid, name: data.title, table_index: 1, id }
    ...
    return (
        <div>
            <Alert type='info' showIcon message='【新增类型】按钮用于增加一级类型；选择目标类型后，点击【新增子类型】创建下属类型' />
            <Row style={styles.root}>
                <Col span={12}>
                    <Button type='primary' size='small' icon='plus' onClick={() => { setSelectKeys(null); setSelectData(null); setAddvisible(true) }}>新增类型</Button>
                ...
                <Col span={12}>
                    {selectData ?
                        <Card title={(selectData ? '当前选择【' + selectData.title + '】' : '')} style={{ width: '100%', marginBottom: 20 }}>
                            <p><Button type='primary' size='small' icon='plus' onClick={() => { setAddvisible(true) }}>新增子类型</Button></p>
                            ...
                            <p><Button type='danger' size='small' icon='delete' onClick={deleteHandler}>删除该类型</Button></p>
                        </Card>
                        : null}
                </Col>
            </Row>
            <AddAttributeModal ref={addAttributeRef} title='新增类型' visible={addvisible} onCancel={() => { setAddvisible(false) }} onOk={() => {
            ...
            <UpdateAttributeModal ref={updateAttributeRef} title='修改类型' treeData={treeDataExceptSelectKey} data={selectData} visible={updatevisible} onCancel={() => { setUpdatevisible(false) }} onOk={() => {
