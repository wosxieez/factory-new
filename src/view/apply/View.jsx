...
export default props => {
    ...
    const getOrderType = useCallback(async () => {
        ...
        if (result.code === 0) {
            setOrderType(result.data[0].map((item, index) => { return { key: index, value: item.id, label: item.order_name } }))
        }
    }, [])
    useEffect(() => {
        getOrderType();
                ...
                <div style={styles.row}>
                    <Row {...rowProps}>
                        ...
                        <Col span={20}>
                            ...
                            >
                                {orderType.map((item, index) => <Option key={index} value={item.value}>{item.label}</Option>)}
                            </Select>
                        </Col>
                    </Row>
                </div>
